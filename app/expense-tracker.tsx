...
type FieldName = "amount" | "description" | "date";
...
export default function ExpenseTracker() {
  ...
  useEffect(() => {
    const frame = window.requestAnimationFrame(() => setDate(getBrowserLocalDate()));
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if ("error" in validation) {
      ...
      setInvalidField(validation.error.startsWith("Enter an amount") ? "amount" : validation.error.startsWith("Enter a description") ? "description" : "date");
      return;
    }
    ...
    try {
      ...
      setAmount(""); setDescription(""); setDate(getBrowserLocalDate()); setSuccessMessage("Expense added.");
      await loadExpenses();
    } catch (error) {
    ...
    <section className="tracker-grid" aria-label="Expense tracker">
      <section className="entry-panel" aria-labelledby="add-expense-heading">
        ...
        <form onSubmit={handleSubmit} noValidate>
          ...
          <div className="field"><label htmlFor="date">Date</label><input id="date" name="date" type="date" value={date} onChange={(event) => { setDate(event.target.value); clearFieldError("date"); }} aria-describedby={invalidField === "date" ? "form-error" : undefined} aria-invalid={invalidField === "date"} required /></div>
